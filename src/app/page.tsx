...
interface Alert {
  ...
  taskName: string;
  department: Department;
  recipientEmail: string; // The dynamic email address
}
...
const initialProjects: Project[] = [
  {
    ...
    tasks: [
      { id: 101, name: "Finalize Collection Designs", assignedTo: { name: "Priya", email: "himu09854@gmail.com" }, timeEstimate: 80, status: "Done", department: "Design", dueDate: "2025-06-10" },
      { id: 102, name: "Approve Production Budget", assignedTo: { name: "Amit", email: "amit.finance.lead@example.com" }, timeEstimate: 16, status: "Done", department: "Finance", dueDate: "2025-06-15" },
      { id: 103, name: "Place Order with Vendor", assignedTo: { name: "Sunita", email: "sunita.supply.lead@example.com" }, timeEstimate: 24, status: "In Progress", department: "Supply", dueDate: "2025-06-25" },
      { id: 104, name: "Plan Photoshoot & Video Content", assignedTo: { name: "Rohan", email: "rohan.mktg.lead@example.com" }, timeEstimate: 40, status: "In Progress", department: "Marketing", dueDate: "2025-07-10" },
      { id: 105, name: "Receive Goods at Warehouse", assignedTo: { name: "Sunita", email: "sunita.supply.lead@example.com" }, timeEstimate: 30, status: "To Do", department: "Supply", dueDate: "2025-07-20" },
    ]
  },
];
...
export default function App() {
      ...
      p.tasks.forEach(t => {
        if (t.status !== 'Done' && t.dueDate < todayStr) {
          ...
          generatedAlerts.push({
            ...
            recipientEmail: t.assignedTo.email // Store the recipient's email
          });
        }
      });
    });
  ...
  const handleSendAlertEmail = async (alert: Alert) => {
    ...
    try {
      const response = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({
          to: alert.recipientEmail, // Use the dynamic email from the alert
  ...
  const handleAddTask = () => {
    ...
    const newTask: Task = {
      id: Date.now(),
      name: taskName,
      assignedTo: { name: assignedToName, email: assignedToEmail }, // Create assignee object
  ...
  if (status === 'loading') { return (<div className="min-h-screen w-full bg-slate-900 text-white flex items-center justify-center"> <Loader className="animate-spin" size={48} /> </div>); }

  const renderContent = () => {
    switch (view) {
      case 'create': return (<div className="w-full max-w-5xl mx-auto"> <button onClick={() => setView('dashboard')} className="flex items-center text-blue-400 hover:text-blue-300 mb-8"><ArrowLeft size={18} className="mr-2" /> Back</button> <div className="bg-slate-800 rounded-xl shadow-2xl p-8"> {createStep === 1 ? (<div> <h2 className="text-2xl font-bold mb-6">Create New Launch Project</h2> <div className="space-y-4"> <InputField icon={<User />} type="text" placeholder="Launch Name" value={newProjectName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewProjectName(e.target.value)} /> <InputField icon={<Calendar />} type="date" value={newProjectLaunchDate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewProjectLaunchDate(e.target.value)} /> </div> <button onClick={() => setCreateStep(2)} disabled={!newProjectName || !newProjectLaunchDate} className="mt-8 w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg disabled:bg-gray-500">Next: Add Tasks</button> </div>) : (<div> <h2 className="text-2xl font-bold mb-2">Add Tasks for "{newProjectName}"</h2> <div className="bg-slate-700 p-4 rounded-lg mb-6"> <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> <InputField icon={<User />} type="text" placeholder="Task Name" value={taskName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTaskName(e.target.value)} /> <SelectField icon={<User />} value={taskDepartment} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setTaskDepartment(e.target.value as Department)}> {DEPARTMENTS.map(d => <option key={d} value={d}>{d}</option>)} </SelectField> <InputField icon={<Clock />} type="number" placeholder="Time Estimate (hrs)" value={timeEstimate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTimeEstimate(e.target.value)} /> <InputField icon={<Calendar />} type="date" value={taskDueDate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTaskDueDate(e.target.value)} /> <InputField icon={<User />} type="text" placeholder="Assignee Name" value={assignedToName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAssignedToName(e.target.value)} /> <InputField icon={<AtSign />} type="email" placeholder="Assignee Email" value={assignedToEmail} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAssignedToEmail(e.target.value)} /> </div> <button onClick={handleAddTask} className="mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg flex items-center"><Plus size={16} className="mr-2" />Add Task</button> </div> <h3 className="font-semibold text-lg mb-4">Tasks Added ({newProjectTasks.length})</h3> <div className="space-y-3 max-h-48 overflow-y-auto pr-2"> {newProjectTasks.map(task => (<div key={task.id} className="bg-slate-700 p-3 rounded-lg flex justify-between items-center"> <span>{task.name} ({task.department})</span> <span className="text-gray-400 text-sm">{task.assignedTo.name} / {task.assignedTo.email}</span> </div>))} </div> <div className="flex justify-between mt-8"> <button onClick={() => setCreateStep(1)} className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-3 px-6 rounded-lg">Back</button> <button onClick={handleCreateProject} className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg">Create Project</button> </div> </div>)} </div> </div>);
      ...
      case 'alerts': return (<div className="w-full max-w-4xl mx-auto"> <button onClick={() => setView('dashboard')} className="flex items-center text-blue-400 hover:text-blue-300 mb-8"><ArrowLeft size={18} className="mr-2" /> Back to Dashboard</button> <h1 className="text-3xl font-bold mb-6">System Alerts</h1> <div className="space-y-4"> {alerts.length > 0 ? alerts.map(alert => (<div key={alert.id} className={`p-4 rounded-lg border-l-4 bg-yellow-900/50 border-yellow-500`}> <div className="flex items-start justify-between"> <div className="flex items-start"> <AlertTriangle className={`mr-4 mt-1 flex-shrink-0 text-yellow-400`} size={24} /> <p className="text-white">{alert.message}</p> </div> <button onClick={() => handleSendAlertEmail(alert)} className="ml-4 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold py-1 px-3 rounded-lg flex items-center gap-2"> <Mail size={14} /> Notify </button> </div> </div>)) : <p className="text-gray-400">No active alerts. All projects are on track!</p>} </div> </div>);
      case 'dashboard':
      default:
        return (<div className="w-full"> <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4"> <div> <h1 className="text-3xl font-bold">Launch Dashboard</h1> <p className="text-gray-400">Welcome, {session?.user?.name || 'User'}!</p> </div> <div className="flex items-center gap-4"> <button onClick={() => setView('alerts')} className="relative bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded-lg flex items-center"> <Mail size={20} className="mr-2" /> Alerts {alerts.length > 0 && <span className="absolute -top-2 -right-2 w-6 h-6 bg-red-600 rounded-full flex items-center justify-center text-xs">{alerts.length}</span>} </button> <button onClick={() => setView('create')} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg flex items-center"><Plus size={20} className="mr-2" /> New Launch</button> <button onClick={() => signOut({ callbackUrl: '/auth/signin' })} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg flex items-center"><LogOut size={20} className="mr-2" /> Sign Out</button> </div> </header> <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"> {projectStats.map(project => (<div key={project.id} onClick={() => handleSelectProject(project)} className={`bg-slate-800 p-6 rounded-xl shadow-lg cursor-pointer hover:bg-slate-700/50 border border-slate-700 transition-all ${project.isAtRisk ? 'border-red-500' : 'hover:border-blue-500'}`}> <div className="flex justify-between items-start"> <h3 className="font-bold text-xl mb-3">{project.name}</h3> {project.isAtRisk && <AlertTriangle className="text-red-500" />} </div> <div className="text-sm text-gray-400 mb-4 space-y-2"> <div className="flex items-center"><Calendar size={14} className="mr-2" /> Launch: {project.launchDate}</div> <div className="flex items-center"><User size={14} className="mr-2" /> {project.taskCount} Tasks</div> <div className="flex items-start"><User size={14} className="mr-2 mt-1" />Depts: <span className="flex flex-wrap gap-1 ml-1">{project.involvedDepts.map(d => <span key={d} className="text-xs bg-slate-700 px-1.5 py-0.5 rounded">{d}</span>)}</span></div> </div> <div className="w-full bg-slate-700 rounded-full h-2.5 mb-2"><div className="bg-green-500 h-2.5 rounded-full" style={{ width: `${project.progress}%` }}></div></div> <p className="text-right text-sm font-semibold text-gray-300">{project.progress}% Complete</p> </div>))} </div> </div>);
    }
  }
